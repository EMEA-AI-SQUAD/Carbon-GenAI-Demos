...
Browser (port 3000)
    â”‚
    â–¼
Next.js app  (IBM Design System UI â€” 9 demo use cases)
    â”‚
    â–¼ port 3001
...
2. **Gather intelligence via IBM Consulting Advantage** (IBM VPN required):
   ...
   - Start a new chat â€” ask it to focus on the customer's country/region
...
3. **Paste the ICA summary into Bob** and say:
   > *"Using this customer intelligence summary, tailor the IBM Power GenAI demo for [customer name]. My audience includes [roles]. Keep the real customer reference scenarios (Hans Geis, Mr. Bean passport) as they are â€” those are useful icebreakers. Update the generic scenarios to reflect this customer's world."*
4. Bob will create a customer-specific branch and update the deployment scripts automatically.
5. Deploy from the customer branch â€” `main` is untouched.

### Which scenarios are safe to tailor

| Demo | Status | Notes |
|------|--------|-------|
| Entity Extraction â€” Book Review / Component Entry | âœ… Tailor freely | Generic by default |
| Entity Extraction â€” IT Ops emails (Italian/French) | âœ… Tailor freely | Generic by default |
| Entity Extraction â€” German Logistics (Hans Geis) | ðŸ”’ Leave as-is | Real IBM customer case study |
| PII â€” Fraud Complaint | âœ… Tailor freely | Generic by default |
| PII â€” Passport Verification (Mr. Bean) | ðŸ”’ Leave as-is | Crowd-pleaser, intentionally light-hearted |
| PII â€” Document Discovery | âœ… Tailor freely | Generic by default |
| Brief Builder | âœ… Tailor freely | Generic by default |
| RFP Assistant | âœ… Tailor freely | Generic by default |
| Talent Acquisition | âœ… Tailor freely | Generic by default |
...
|-----|----------|----------------|
| Entity Extraction | ðŸ“¦ Component / Product Entry | Product description text |
